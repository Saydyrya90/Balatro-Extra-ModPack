LOVELY_INTEGRITY = 'ff98beecfe4cbe7d2f707d6820bf7ca16ca233300b7672466b17a4b66eed5b0a'
...
function CardArea:add_to_highlighted(card, silent)
    ...
    elseif self.config.type == 'joker' or self.config.type == 'consumeable' then
        if (#self.highlighted >= self.config.highlighted_limit) and (not card.ability.group) and (G.GAME and not G.GAME.THE_8_BYPASS) then
            self:remove_from_highlighted(self.highlighted[1])
        end
        self.highlighted[#self.highlighted+1] = card
        card:highlight(true)
