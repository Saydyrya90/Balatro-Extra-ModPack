LOVELY_INTEGRITY = 'e190fe79d0dc38fedb7104cebf5d508a901b80134e923d7b2669558a04b84390'
	...
	for _, mod in ipairs(SMODS.mod_list) do
		if mod.set_debuff and type(mod.set_debuff) == 'function' then
            ...
            if res == 'prevent_debuff' then
                if self.debuff or cry_get_gameset(self) == 'disabled' or (self.config.center.gameset_config and self.config.center.gameset_config[cry_get_gameset(self)] and self.config.center.gameset_config[cry_get_gameset(self)].disabled) or (self.config.center.cry_disabled and not self.gameset_select) then
                    self.debuff = false
                    if self.area == G.jokers then self:add_to_deck(true) end
...
function Card:remove()
    ...
    if self.ability.bunc_scattering and self.area == G.jokers then
        ...
        if position then
            scattered_jokers = {}
            if G.jokers.cards[position - 1] then table.insert(scattered_jokers, G.jokers.cards[position - 1]) end
            if G.jokers.cards[position + 1] then table.insert(scattered_jokers, G.jokers.cards[position + 1]) end
        end
    end
